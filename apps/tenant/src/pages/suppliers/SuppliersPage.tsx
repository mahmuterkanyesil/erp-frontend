...
import { useSuppliers, SupplierTable } from "@/features/suppliers"
...
export function SuppliersPage() {
  ...
  function handleRowClick(row: Supplier) {
    navigate({ to: "/suppliers/$supplierId", params: { supplierId: row.id } })
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      <PageHeader
        ...
        actions={
          <PermissionGate permission="customers:create">
            <Button leftIcon="add" onClick={() => navigate({ to: "/suppliers/new" })}>
              {t("newSupplier")}
            </Button>
          </PermissionGate>
        }
      ...
      <SupplierTable data={data} loading={isLoading} onRowClick={handleRowClick} />
    </div>
  )
}
